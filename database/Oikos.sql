...
-- Recurring payment templates (defined before Transactions so the FK can reference it)
CREATE TABLE IF NOT EXISTS Recurring_Templates (
    ...
    Auto_Confirm        BOOLEAN DEFAULT FALSE,  -- TRUE = bypass review queue. Mutually exclusive
                                                 -- with Total_Occurrences below (installments are
                                                 -- generated by confirming the prior one -- see
                                                 -- database/queries.py::_confirm_draft_row -- so
                                                 -- an installment template that also auto-confirms
                                                 -- would never trigger the rest of the series).
    Active              BOOLEAN DEFAULT TRUE,
    Accounts_Id_Target  INTEGER REFERENCES Accounts(Accounts_Id),
    Created_At          TIMESTAMPTZ DEFAULT NOW(),
    Total_Occurrences   INTEGER,                -- NULL = every generated draft stands alone (default).
                                                 -- NOT NULL = each cycle's draft, once confirmed,
                                                 -- expands into a fixed-count installment series of
                                                 -- N-1 more drafts spaced by Installment_Frequency.
                                                 -- The template itself is untouched by this -- it
                                                 -- keeps recurring on Periodicity/Next_Due_Date/
    ...
    Installment_Frequency VARCHAR(20),          -- Spacing between the N installments within one
                                                 ...
                                                 -- Required (app-level) whenever Total_Occurrences
                                                 -- is set; same vocabulary as Periodicity minus
                                                 -- 'Once'.
    CONSTRAINT recurring_templates_total_occurrences_check
...
CREATE TABLE Transactions (
    ...
    Templates_Id            INTEGER REFERENCES Recurring_Templates(Templates_Id) ON DELETE SET NULL,
    Installment_Group_Id    INTEGER,                 -- shared value links every row of one
                                                       -- installment series -- mirrors Transfers_Id
                                                       -- above (no FK: set to the confirmed starter
                                                       -- row's own Transactions_Id). NULL until a
                                                       -- Total_Occurrences template's draft is
                                                       -- actually confirmed.
    Installment_Seq         INTEGER,                  -- 1 on the starter, 2..N on the generated
                                                       -- rows within one Installment_Group_Id.
    embedding               vector(768)
);
...
CREATE        INDEX IF NOT EXISTS idx_transactions_draft    ON Transactions(Is_Draft) WHERE Is_Draft = TRUE;
...
-- =============================================================================
-- USERS & SESSIONS  (login auth — see database/migrations/014_users_sessions.sql)
-- =============================================================================
...
-- Sessions.Token_Hash stores a SHA-256 hash of the session cookie's random token,
-- never the raw token itself — a leaked DB backup shouldn't hand over live sessions.
